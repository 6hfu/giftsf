...
def aggregate_calls(records):
    ...
    data = {
        "today": create_day_dict(),
        "tomorrow": create_day_dict()
    }
    ...
    for r in records:
        ...
        status = r.get("Field109__c")

        if not call_date or not call_time or status not in TARGET_STATUSES:
            continue

        try:
            call_date = datetime.strptime(call_date, "%Y-%m-%d").date()
            hour = int(call_time[:2])
        except:
            continue
        ...
        if call_date == today:
            data["today"]["hours"][hour][status][category] += 1
            data["today"]["total"] += 1

        elif call_date == tomorrow:
            data["tomorrow"]["hours"][hour][status][category] += 1
